...
// Sweep expired sessions periodically so the Map doesn't grow forever on a
// long-running process — without this, every login (even ones that later
// expire) stays in memory until the process restarts.
setInterval(() => {
  const now = Date.now();
...
// ---------- login rate limiting ----------
// The admin login has no username, just a password — without a limit on
// attempts, it's a straightforward target for brute force. This tracks
// failed attempts per IP and locks that IP out temporarily after too many.
...
// Applied to every response, API or static. Keeps things sane by default:
// no MIME-sniffing, no framing by other sites, and a conservative referrer.
// The CSP allows what the storefront actually needs (Google Fonts, https
// product images, the small inline theme script) and nothing else.
...
function gzipIfAccepted(req, res, buffer, extraHeaders) {
  const acceptsGzip = (req.headers['accept-encoding'] || '').includes('gzip');
  const headers = Object.assign({}, SECURITY_HEADERS, extraHeaders || {});
  if (acceptsGzip && buffer.length > 1024) {
...
function sendJSON(res, status, data, extraHeaders) {
  ...
  // res.req is set by Node on every response, so callers don't need to
  // thread the request object through just for the Accept-Encoding check.
  const { body, headers } = gzipIfAccepted(res.req || { headers: {} }, res, buffer, Object.assign({
...
function setSessionCookie(req, res, token) {
  ...
  res.setHeader('Set-Cookie', `drift_session=${token}; HttpOnly; Path=/; Max-Age=${maxAge}; SameSite=Lax${secure}`);
}
...
function sanitizeProductInput(body) {
  ...
  if (!Number.isFinite(price) || price < 0 || price > 1_000_000) {
    throw new Error('Price must be a positive number under 1,000,000.');
  }
...
// HTML/JSON get short-lived caching so config or template edits show up
// promptly; assets like icons and fonts are safe to cache aggressively.
function cacheControlFor(ext) {
  if (ext === '.html') return 'no-cache';
  if (['.png', '.svg', '.jpg', '.jpeg', '.webp', '.gif', '.ico', '.woff', '.woff2'].includes(ext)) {
...
function serveStatic(req, res, pathname) {
  ...
  fs.readFile(fullPath, (err, data) => {
    ...
    if (compressible) {
      const { body, headers } = gzipIfAccepted(req, res, data, staticHeaders);
      res.writeHead(200, headers);
      res.end(body);
    } else {
      res.writeHead(200, Object.assign({ 'Content-Length': data.length }, SECURITY_HEADERS, staticHeaders));
      res.end(data);
    }
  });
}
