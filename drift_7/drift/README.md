...
## Navigation, search, and dark mode

The top bar now has:

- **shop** / **thrifted** links that jump to each section (the "thrifted"
  link only appears once you've marked at least one item as thrifted)
- A **search icon (⌕)** that drops down a search bar — it filters items live
  by name, description, or category as the customer types
- A **dark mode toggle (◐)** — switches the whole site to a dark palette,
  and remembers the visitor's choice next time

## Thrifted section

Any item marked **"thrifted item"** in `/admin` (a checkbox in the item
editor) shows up in its own **"thrifted finds"** section on the storefront,
separate from the main collection — good for one-off or secondhand pieces
you want to visually set apart from your regular stock. Leave it unchecked
and the item just appears in the main collection as usual.

## How payment confirmation already works

Just to be clear on something that's easy to miss: when a customer chooses
**pay online now**, the order details are only ever sent to your WhatsApp
*after* Paystack confirms the payment actually succeeded — never before. If
a customer starts the online checkout and abandons it, or the payment fails
or gets declined, no WhatsApp message is sent at all. You'll only ever see a
WhatsApp order for an online payment once it's genuinely gone through. For
**pay on delivery**, there's no online payment step at all by design — that
option exists specifically for customers who want to pay in person instead.

## Photos, colors, and sizes

Each item can have up to 6 photos that customers swipe through, plus
optional colors and sizes. In `/admin`, when adding or editing an item:

- **Photos:** upload files or paste image URLs — mix and match. The first
  photo is what shows on the shop grid; all of them appear as a swipeable
  gallery when a customer opens the item.
- **Colors / sizes:** type them separated by commas, e.g. `Black, Sand, Rust`
  or `S, M, L, XL`. Leave either blank if it doesn't apply — an item with no
  colors or sizes just shows a plain "add to bag" button; add either one and
  customers are taken to the item's page to choose before adding it to
  their bag.
